...
(function() {
 var loadPackage = function(metadata) {
    ...
    var PACKAGE_NAME = 'cashgettools_wasm.data';
    var REMOTE_PACKAGE_BASE = 'cashgettools_wasm.data';
...
Module['FS_createPath']('/data', 'CW_mimetypes', true, true);
        ...
        DataRequest.prototype.byteArray = Module['HEAPU8'].subarray(ptr, ptr+byteArray.length);
          ...
          for (var i = 0; i < files.length; ++i) {
            DataRequest.prototype.requests[files[i].filename].onload();
              ...
              Module['removeRunDependency']('datafile_cashgettools_wasm.data');

    };
    Module['addRunDependency']('datafile_cashgettools_wasm.data');
 ...
 loadPackage({"files": [{"filename": "/data/CW_mimetypes/CW0_mime.types", "start": 0, "end": 60847, "audio": 0}], "remote_package_size": 60847, "package_uuid": "a2eb8948-ab99-4ce5-b159-ca53191eca3f"});
...
var STATIC_BASE = 1024,
    ...
    STACKTOP = STACK_BASE,
    STACK_MAX = 13312,
    DYNAMIC_BASE = 5256192,
    DYNAMICTOP_PTR = 13296;
...
var wasmBinaryFile = 'cashgettools_wasm.wasm';
...
var ASM_CONSTS = [];
function jsFetch(url,isPost,postData,reqLimit,fTmpNam){ const urlStr = UTF8ToString(url); const tmpNamStr = UTF8ToString(fTmpNam); const headersJ = reqLimit ? { "X-Forwarded-For": Math.floor(Math.random() * 1000).toString(10) + "." + Math.floor(Math.random() * 1000) + "." + Math.floor(Math.random() * 1000) + "." + Math.floor(Math.random() * 100), "Accept": 'application/json', "Content-Type": 'application/json' } : { "Accept": 'application/json', "Content-Type": 'application/json' }; Asyncify.handleSleep(function(wakeUp) { fetch(urlStr, !isPost ? { method: "GET", credentials: "omit", headers: headersJ } : { method: "POST", body: UTF8ToString(postData), credentials: "omit", headers: headersJ }).then(response => response.text()).then(text => { FS.writeFile(tmpNamStr, text); wakeUp(); }).catch(error => { FS.writeFile(tmpNamStr, ""); wakeUp(); }); }); }



// STATICTOP = STATIC_BASE + 12288;
...
// {{PRE_LIBRARY}}
  ...
  try {
    ...
    return -e.errno;
      ...
      if (Asyncify.state !== originalAsyncifyState) throw "import fd_write was not in ASYNCIFY_IMPORTS, but changed the state";
    }
  }
  ...
  var Asyncify={State:{Normal:0,Unwinding:1,Rewinding:2},state:0,StackSize:4096,currData:null,dataInfo:{},handleSleepReturnValue:0,exportCallStack:[],afterUnwind:null,asyncFinalizers:[],instrumentWasmExports:function(exports) {
        var ret = {};
...
var asmGlobalArg = {};
var asmLibraryArg = { "DYNAMICTOP_PTR": DYNAMICTOP_PTR, "__assert_fail": ___assert_fail, "__clock_gettime": ___clock_gettime, "__lock": ___lock, "__setErrNo": ___setErrNo, "__syscall10": ___syscall10, "__syscall140": ___syscall140, "__syscall145": ___syscall145, "__syscall195": ___syscall195, "__syscall20": ___syscall20, "__syscall221": ___syscall221, "__syscall3": ___syscall3, "__syscall33": ___syscall33, "__syscall4": ___syscall4, "__syscall5": ___syscall5, "__syscall54": ___syscall54, "__syscall6": ___syscall6, "__unlock": ___unlock, "abort": _abort, "abortOnCannotGrowMemory": abortOnCannotGrowMemory, "clock_gettime": _clock_gettime, "demangle": demangle, "demangleAll": demangleAll, "emscripten_get_heap_size": _emscripten_get_heap_size, "emscripten_get_now": _emscripten_get_now, "emscripten_get_now_is_monotonic": _emscripten_get_now_is_monotonic, "emscripten_memcpy_big": _emscripten_memcpy_big, "emscripten_resize_heap": _emscripten_resize_heap, "emscripten_set_main_loop": _emscripten_set_main_loop, "emscripten_set_main_loop_timing": _emscripten_set_main_loop_timing, "fd_write": _fd_write, "gettimeofday": _gettimeofday, "jsFetch": jsFetch, "jsStackTrace": jsStackTrace, "memcpy": _memcpy, "memset": _memset, "runAndAbortIfError": runAndAbortIfError, "sbrk": _sbrk, "sched_yield": _sched_yield, "setTempRet0": _setTempRet0, "stackTrace": stackTrace, "time": _time };
...
var real__CWG_WA_get_by_id_bitdb = asm["CWG_WA_get_by_id_bitdb"];
asm["CWG_WA_get_by_id_bitdb"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return real__CWG_WA_get_by_id_bitdb.apply(null, arguments);
};

var real__CWG_WA_get_by_id_rest = asm["CWG_WA_get_by_id_rest"];
asm["CWG_WA_get_by_id_rest"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return real__CWG_WA_get_by_id_rest.apply(null, arguments);
};

var real__CWG_WA_errno_print_msg = asm["CWG_WA_errno_print_msg"];
asm["CWG_WA_errno_print_msg"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return real__CWG_WA_errno_print_msg.apply(null, arguments);
};
...
var _CWG_WA_get_by_id_bitdb = Module["_CWG_WA_get_by_id_bitdb"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return Module["asm"]["CWG_WA_get_by_id_bitdb"].apply(null, arguments)
};

var _CWG_WA_get_by_id_rest = Module["_CWG_WA_get_by_id_rest"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return Module["asm"]["CWG_WA_get_by_id_rest"].apply(null, arguments)
};

var _CWG_WA_errno_print_msg = Module["_CWG_WA_errno_print_msg"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return Module["asm"]["CWG_WA_errno_print_msg"].apply(null, arguments)
};
