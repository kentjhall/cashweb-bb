...
(function() {
 var loadPackage = function(metadata) {
    ...
    var PACKAGE_NAME = 'cashgettools.data';
    var REMOTE_PACKAGE_BASE = 'cashgettools.data';
...
Module['FS_createPath']('/data', 'CW_mimetypes', true, true);
        ...
        DataRequest.prototype.byteArray = Module['HEAPU8'].subarray(ptr, ptr+byteArray.length);
          ...
          for (var i = 0; i < files.length; ++i) {
            DataRequest.prototype.requests[files[i].filename].onload();
              ...
              Module['removeRunDependency']('datafile_cashgettools.data');

    };
    Module['addRunDependency']('datafile_cashgettools.data');
 ...
 loadPackage({"files": [{"filename": "/data/CW_mimetypes/CW0_mime.types", "start": 0, "end": 60847, "audio": 0}], "remote_package_size": 60847, "package_uuid": "a4805a1a-c869-4353-bb8a-efc338209b8a"});
...
var wasmBinaryFile = 'cashgettools.wasm';
...
var ASM_CONSTS = [];
function jsFetch(url,reqLimit,fTmpNam){ var urlStr = UTF8ToString(url); var tmpNamStr = UTF8ToString(fTmpNam); Asyncify.handleSleep(function(wakeUp) { fetch(urlStr, { credentials: "omit", headers: (reqLimit ? { "X-Forwarded-For": Math.floor(Math.random() * 1000).toString(10) + "." + Math.floor(Math.random() * 1000) + "." + Math.floor(Math.random() * 1000) + "." + Math.floor(Math.random() * 100) } : {}) }).then(response => response.text()).then(text => { FS.writeFile(tmpNamStr, text); wakeUp(); }).catch(error => { FS.writeFile(tmpNamStr, ""); wakeUp(); }); }); }
...
var real__CWG_WA_get_by_id = asm["CWG_WA_get_by_id"];
asm["CWG_WA_get_by_id"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return real__CWG_WA_get_by_id.apply(null, arguments);
};
...
var _CWG_WA_get_by_id = Module["_CWG_WA_get_by_id"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return Module["asm"]["CWG_WA_get_by_id"].apply(null, arguments)
};
